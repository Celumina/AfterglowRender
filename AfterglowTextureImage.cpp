...
void AfterglowTextureImage::bind(const img::Info& imageInfo, std::weak_ptr<void> imageData) {
	if (_memory) {
...
std::weak_ptr<void> AfterglowTextureImage::imageData() {
	return _imageData;
}
...
void AfterglowTextureImage::submit(AfterglowCommandPool& commandPool, AfterglowGraphicsQueue& graphicsQueue) {
	...
	AfterglowStagingBuffer stagingBuffer(_device, lockedPtr.get(), size());
