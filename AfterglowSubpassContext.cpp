...
VkAttachmentDescription AfterglowSubpassContext::depthAttachment(VkFormat format, VkSampleCountFlagBits sampleCount) {
	...
	attachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
	attachment.finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL; //VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
	return attachment;
}
...
bool AfterglowSubpassContext::bindInputAttachment(render::Domain domain, uint32_t attachmentIndex, const std::string& attachmentName) {
	if (!bindAttachment(
		AttachmentReferenceArrayID::Input, 
		domain, 
		attachmentIndex, 
...
bool AfterglowSubpassContext::bindDepthAttachment(render::Domain domain, uint32_t attachmentIndex) {
	...
	info->depthAttachmentRef.layout =
...
uint32_t AfterglowSubpassContext::appendDepthAttachment(VkAttachmentDescription&& attachment) {
	...
	_clearValues[index] = VkClearValue{ {1.0f, 0} };
	return index;
}
