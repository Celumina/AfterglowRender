...
public:
	...
	// @brief: Find first component which is match the name in the current scene.
	AfterglowEntity* findEntity(const std::string& name) const;
	std::vector<AfterglowEntity*> findEntities(const std::string& name) const;

	// Quick create entity functions.
	AfterglowEntity& createStaticMeshEntity(const std::string& modelPath, const std::string& materialName) const;
	AfterglowComponentBase& addComponent(AfterglowEntity& destEntity, std::type_index componentTypeIndex) const;

	AfterglowCameraComponent* mainCamera() const;
...
INR_CLASS(AfterglowSystemUtilities) {
	INR_FUNCS (
		...
		INR_FUNC(findEntity), 
		INR_FUNC(findEntities), 
		INR_FUNC(createStaticMeshEntity), 
		INR_FUNC(addComponent), 
		INR_FUNC(mainCamera), 
		// INR_FUNC(input),
		INR_FUNC(lockCursor), 
