...
public: 
	...
	uint32_t byteSizeWithoutPadding() const;
...
private:
	...
	inline void updateAttributeAlignmentStates(const uint32_t attributeSize, uint32_t& size, uint32_t& currentGroupSize) const noexcept;
	...
	static inline const std::unordered_map<AttributeType, AttributeInfo> _attributeTypeInfos = {
		...
		{AttributeType::Float2x2, {"float2x2", 16, 4}}, // e.g. glm::mat2x2
		{AttributeType::Float3x3, {"float4x3", 48, 9}}, // e.g. glm::mat3x4 for memory alignment, noticed that hlsl has inversed rows and column with glm.
		{AttributeType::Float4x4, {"float4x4", 64, 16}},// e.g. glm::mat4x4
