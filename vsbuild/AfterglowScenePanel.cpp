...
struct AfterglowScenePanel::Impl {
	...
	// @note: Do not use the specialized vector<bool>
	std::vector<uint8_t> sceneTreeStack;
...
inline void AfterglowScenePanel::Impl::pushSceneTreeNode(AfterglowEntity& entity) {
	...
	if (&entity == activeEntity) {
		...
		sceneTreeStack.push_back(ImGui::TreeNodeEx(entity.name().data(), flags));
		ImGui::PopStyleColor();
	}
	else {
		sceneTreeStack.push_back(ImGui::TreeNodeEx(entity.name().data(), flags));
...
inline void AfterglowScenePanel::Impl::popSceneTreeNode() {
	if (sceneTreeStack.back()) {
		ImGui::TreePop();
	}
	sceneTreeStack.pop_back();
}

inline bool AfterglowScenePanel::Impl::shouldShowTreeSubnodes() {
	return sceneTreeStack.back();
}
