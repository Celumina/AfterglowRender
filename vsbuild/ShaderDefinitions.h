...
namespace shader {
	...
	INR_CLASS(SetIndex) {
		INR_ATTRS(
			...
			INR_ENUM(ComputeShared), 
			INR_ENUM(ExternalStorage)
		);
	};
	...
	constexpr auto computeSetIndexBegin = std::underlying_type_t<SetIndex>(SetIndex::Compute);
	constexpr auto computeSetIndexEnd = std::underlying_type_t<SetIndex>(SetIndex::ComputeShared) + 1;
	...
	enum class Stage : uint32_t {
		...
		ComputeShared = SetIndex::ComputeShared, 
	...
	enum class GlobalSetBindingIndex : uint32_t {
		...
		ambientTexture = 1, 
		diffuseEnergyTexture = 2, 

		// TODO: Shared samplers
		// sharedNearestSampler
		// sharedBilinearSampler
		// sharedTrilinearSampler
		// sharedAnisotropicSampler
