...
UnlitVSOutput main(VSInput input) {
	...
	float distanceFade = min(distance(cameraPosition.xyz, worldPosition.xyz) * outlineDistanceFadeFactor, 0.001);
	float fovFade = invProjection[0][0] * invScreenAspectRatio;

	half4 st = stTex.SampleLevel(stTexSampler, input.texCoord0, 0);
	
	worldPosition.xyz += outlineWidth * distanceFade * fovFade * worldNormal * st.x;
