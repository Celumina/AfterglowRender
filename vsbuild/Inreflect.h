...
template<> struct InreflectSpecializationTag<Type>{}; \
template<> struct Inreflect<Type> : public InreflectDefinition<Type>
...
struct InreflectSpecializationTag;
...
namespace inreflect {
	...
	template <typename Type>
	struct InreflectTrait : std::false_type {};

	template <typename Type>
	struct InreflectTrait<Inreflect<Type>> : std::true_type {};

	// Verify the Type is a instantiated Inreflect Type.
	template <typename Type>
	concept InreflectType = InreflectTrait<Type>::value;

	// Verify the Type from Inreflect<Type> is is a specialized type.
	template <typename Type>
	concept ReflectibleType = !requires {
		typename InreflectSpecializationTag<Type>::Unspecialized;
	};

	template<typename Type>
	concept ReflectibleEnumType = ReflectibleType<Type> && std::is_enum_v<Type>;
	...
	// DecayTuple
	template <constraint::TupleType Type>
	struct PureTupleTrait;

	template <typename... Types>
	struct PureTupleTrait<std::tuple<Types...>> {
		using Tuple = std::tuple<std::remove_reference_t<std::remove_pointer_t<std::decay_t<Types>>>...>;
	};

	template <constraint::TupleType Type>
	using PureTuple = PureTupleTrait<Type>::Tuple;

	// PointerTuple
	template <constraint::TupleType Type>
	struct PointerTupleTrait;

	template <typename... Types>
	struct PointerTupleTrait<std::tuple<Types...>> {
		using Tuple = std::tuple<std::add_pointer_t<Types>...>;
	};

	template <constraint::TupleType Type>
	using PointerTuple = PointerTupleTrait<PureTuple<Type>>::Tuple;

	template<constraint::TupleType Type, size_t... indices>
...
struct InreflectFunctionBase : public InreflectMemberBase<FunctionType, NameType, isStaticFunction> {
	...
	// decltype(auto) for Perfect Forwarding (return reference)
	static decltype(auto) parseParam(Type& val) noexcept {
		using Param = std::tuple_element_t<paramIndex, ParamTuple>;
		if constexpr (std::is_pointer_v<Param> && !std::is_pointer_v<Type>) {
			return &val;
		}
		else if constexpr (!std::is_pointer_v<Param> && std::is_pointer_v<Type>) {
			return static_cast<std::remove_pointer_t<Type>&>(*val);
		}
		else {
	...
	// @brief: Let pure tuple can be used in call().
	template<constraint::TupleType Type, size_t ...indices>
	static auto parseParamTuple(Type& srcTuple, std::index_sequence<indices...>) {
		return std::forward_as_tuple(parseParam<indices>(std::get<indices>(srcTuple))...);
	};

	template<constraint::TupleType Type>
	static auto makeParsedParamTuple(Type& srcTuple) {
		return parseParamTuple(srcTuple, std::make_index_sequence<std::tuple_size_v<Type>>{});
	}
};
...
struct InreflectMemberFunction : public InreflectFunctionBase<FunctionType, NameType, functionAddress, false> {
	using Parent = InreflectFunctionBase<FunctionType, NameType, functionAddress, false>;
	using Class = FunctionTraits<FunctionType>::Class;
	using Return = FunctionTraits<FunctionType>::Return;
	using ParamTuple = FunctionTraits<FunctionType>::ParamTuple;
	// using PureParamTuple = typename inreflect::PureTuple<ParamTuple>;
	
	template<typename ...ParamTypes>
	static constexpr Return call(Class& instance, ParamTypes&& ...params) {
		return (instance.*functionAddress)(params...);
	}
	
	template<typename ...ParamTypes>
	static constexpr Return call(const Class& instance, ParamTypes&& ...params) {
		return (instance.*functionAddress)(params...);
	}

	static constexpr Return call(Class& instance, ParamTuple& params) {
		return std::apply([&instance](auto&&... args) -> Return {
			return (instance.*functionAddress)(std::forward<decltype(args)>(args)...);
		}, params);
	}

	static constexpr Return call(const Class& instance, ParamTuple& params) {
		return std::apply([&instance](auto&&... args) -> Return {
			return (instance.*functionAddress)(std::forward<decltype(args)>(args)...);
		}, params);
	}

	// For other type tuples
	template<constraint::TupleType Type>
	static constexpr Return call(Class& instance, Type& params) {
		return std::apply([&instance](auto&&... args) -> Return {
			return (instance.*functionAddress)(std::forward<decltype(args)>(args)...);
		}, Parent::makeParsedParamTuple(params));
	}

	template<constraint::TupleType Type>
	static constexpr Return call(const Class& instance, Type& params) {
		return std::apply([&instance](auto&&... args) -> Return {
			return (instance.*functionAddress)(std::forward<decltype(args)>(args)...);
		}, Parent::makeParsedParamTuple(params));
	}
...
struct InreflectStaticFunction : public InreflectFunctionBase<FunctionType, NameType, functionAddress, true> {
	using Parent = InreflectFunctionBase<FunctionType, NameType, functionAddress, true>;
	using Return = FunctionTraits<FunctionType>::Return;
	using ParamTuple = FunctionTraits<FunctionType>::ParamTuple;
	// using PureParamTuple = typename inreflect::PureTuple<ParamTuple>;
	...
	// For other type tuples 
	template<constraint::TupleType Type>
	static constexpr Return call(Type& params) {
		return std::apply([](auto&&... args) -> Return {
			return functionAddress(std::forward<decltype(args)>(args)...);
		}, Parent::makeParsedParamTuple(params));
	}
