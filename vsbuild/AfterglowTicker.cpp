...
inline void AfterglowTicker::Impl::tick() {
	...
	int64_t timeSinceLastUpdate = clock.timeSinceLastUpdate<unit::Nanoseconds>();
	if (timeSinceLastUpdate < frameInterval) {
		int64_t sleepTime = std::max(frameInterval - timeSinceLastUpdate - sleepDelay, int64_t(0));
