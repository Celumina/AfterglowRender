...
struct AfterglowRenderer::Context {
	Context(AfterglowWindow& window); 
...
void AfterglowRenderer::startRenderThread() {
	if (!_context->renderThread) {
		_context->renderThread = std::make_unique<std::jthread>([&]() { renderLoop(); });
	}
}
...
void AfterglowRenderer::renderLoop() {
	if (!_context->renderableContext) {
...
void AfterglowRenderer::draw() {
	auto& synchronizer = *_context->synchronizer;
	auto& commandManager = *_context->commandManager;
	...
	_context->computeQueue->submit(commandManager.computeCommandBuffers(), synchronizer);

	// Render Submission: 
	if (!_window.drawable()) {
		// TODO: Optional compute task type for window relativity.
		_context->computeQueue->cancelSemaphore(synchronizer);
		return;
	}

	auto& framebufferManager = *_context->framebufferManager;
	...
	auto materialManagerLock = _context->materialManager->lock();
	...
	_context->graphicsQueue->submit(commandManager.drawCommandBuffers(), synchronizer);
	_context->presentQueue->submit(_window, framebufferManager, synchronizer, imageIndex);
}

void AfterglowRenderer::completeSubmission() {
	_context->assetMonitor.update(_context->clock);
	(*_context->device).updateCurrentFrameIndex();
}

void AfterglowRenderer::evaluateRenderable() {	
	auto& renderableContext = *_context->renderableContext;
	...
	renderableContext.camera->setAspectRatio((*_context->swapchain).aspectRatio());
	
	// PostProcess
	// TODO: Handle post process disable.
	if (renderableContext.postProcess) {
		_context->meshManager->shapeMesh(_context->ndcMeshIndex).materialName = 
	...
	_context->meshManager->update(renderableContext.componentPool, *renderableContext.camera);
	...
	_context->materialManager->update(_context->framebufferManager->imageWriteInfos(), *_context->synchronizer);

	recordDraws();
}


void AfterglowRenderer::evaluateComputable() {
	auto& renderableContext = *_context->renderableContext;
	...
	auto& meshManager = *_context->meshManager;
...
void AfterglowRenderer::submitMeshUniforms() {
	auto& meshResources = _context->meshManager->meshResources();
	auto& materialManager = *_context->materialManager;
	// Scene meshes
	...
	auto& shapeMeshes = _context->meshManager->shapeMeshes();
...
void AfterglowRenderer::recordDraws() {
	auto& materialManager = *_context->materialManager;
	auto& meshResources = _context->meshManager->meshResources();
	...
	const auto& computeMeshInfos = _context->meshManager->computeMeshInfos();
	for (const auto& [id, computeMeshInfo] : computeMeshInfos) {
		const auto& materialName = *computeMeshInfo.materialName;
		...
		_context->commandManager->recordDraw(
			*matResource, 
			*setRefs,
...
inline bool AfterglowRenderer::recordDraw (
...
inline void AfterglowRenderer::recordDispatch(const std::string& materialName, const ubo::MeshUniform& meshUniform) {
	auto& commandManager = *_context->commandManager;
	auto& materialManager = *_context->materialManager;
...
void AfterglowRenderer::updateGlobalUniform() {
	auto& globalUniform = _context->materialManager->globalUniform();
	auto& renderableContext = *_context->renderableContext;
	...
	globalUniform.cursorPosition = _window.input().cursorPosition();
	// Here replace as render clock instead of logic clock (For uniform shader animation).
	globalUniform.time = static_cast<float>(_context->clock.timeSec());
	globalUniform.deltaTime = static_cast<float>(_context->clock.deltaTimeSec());
	globalUniform.cameraNear = renderableContext.camera->near();
	globalUniform.cameraFar = renderableContext.camera->far();
}

AfterglowRenderer::Context::Context(AfterglowWindow& window) {
