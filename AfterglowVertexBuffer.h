...
public:
	...
	void bind(std::weak_ptr<VertexArray> vertices);
	std::weak_ptr<const VertexArray> vertexData() const;
...
private:
	...
	inline std::shared_ptr<VertexArray> safeVertices();

	std::weak_ptr<VertexArray> _vertices;
	Info _info;
};
...
inline void AfterglowVertexBufferTemplate<Type>::set(Size pos, const vert::Vec4<Attribute>& value) {
	(*safeVertices())[pos].set<Attribute>(Parent::value);
}
...
template<vert::VertexType Type>
inline void AfterglowVertexBufferTemplate<Type>::bind(std::weak_ptr<VertexArray> vertices) {
	if (Parent::_memory) {
		throw Parent::runtimeError("Indices had been created early.");
	}
	_vertices = vertices;
	_info.count = static_cast<Size>(safeVertices()->size());
...
template<vert::VertexType Type>
inline std::weak_ptr<const typename AfterglowVertexBufferTemplate<Type>::VertexArray> AfterglowVertexBufferTemplate<Type>::vertexData() const {
	return _vertices;
}

template<vert::VertexType Type>
inline uint64_t AfterglowVertexBufferTemplate<Type>::byteSize() {
	return sizeof(Vertex) * safeVertices()->size();
}
...
inline void AfterglowVertexBufferTemplate<Type>::submit(AfterglowCommandPool& commandPool, AfterglowGraphicsQueue& graphicsQueue) {
	...
	AfterglowStagingBuffer stagingBuffer(Parent::_device, safeVertices()->data(), byteSize());
...
template<vert::VertexType Type>
inline std::shared_ptr<typename AfterglowVertexBufferTemplate<Type>::VertexArray> AfterglowVertexBufferTemplate<Type>::safeVertices() {
	auto lockedPtr = _vertices.lock();
	if (!lockedPtr) {
		throw Parent::runtimeError("Vertices was not found, due to vertex data source was destructed.");
	}
	return lockedPtr;
}
